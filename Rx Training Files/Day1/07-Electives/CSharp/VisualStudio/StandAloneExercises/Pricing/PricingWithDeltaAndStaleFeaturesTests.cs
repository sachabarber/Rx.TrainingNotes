...
namespace StandAloneExercises.Pricing
{
    ...
    public class PricingWithDeltaAndStaleFeaturesTests : ReactiveTest
    {
        ...
        public void PriceFeedIsEnrichedWithDeltaFromPreviousPrice()
        {
            ...
            var actual = _priceFeed.Select(x => new PriceTick() {Price = x.Price})
                .Scan(
                (tick1, tick2) => new PriceTick()
                {
                    Price = tick2.Price,
                    Delta = tick2.Price - tick1.Price
                });
        ...
        public void PriceFeedGoesStaleAfter2Seconds()
        {
            ...
            //TODO: Put implementation here
            var actual = _priceFeed.Select(x =>
                    Observable.Return(new PriceTick() { Price = x.Price, IsValid = true })
                    .Merge(Observable.Timer(TimeSpan.FromSeconds(2), _testScheduler).Select(_ => new PriceTick() { Price = x.Price })))
                .Switch();
        ...
        {
            //TODO: Put implementation here
            var staleStream = _priceFeed.Select(x =>
                   Observable.Return(new PriceTick() { Price = x.Price, IsValid = true })
                   .Merge(Observable.Timer(TimeSpan.FromSeconds(2), _testScheduler)
                   .Select(_ => new PriceTick() { Price = x.Price })))
               .Switch();

            var actual = staleStream.Select(x => new PriceTick() { Price = x.Price, IsValid = x.IsValid})
              .Scan(
              (tick1, tick2) => new PriceTick()
              {
                  Price = tick2.Price,
                  IsValid = tick2.IsValid,
                  Delta = tick2.Price - tick1.Price
              });
