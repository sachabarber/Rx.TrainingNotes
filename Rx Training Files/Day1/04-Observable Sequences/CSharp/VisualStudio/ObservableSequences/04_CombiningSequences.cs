...
{
    [TestFixture]
    public class CombiningSequences : ReactiveTest
    {
        [Test]
        ...
        public void Test_Zipping_two_size_equal_sequences()
        {
            ...
            var query = s1.Zip(s2, (x, y) => Tuple.Create<int, char>(x, y));
        ...
        public void Test_CartesianProduct()
        {
            ...
            var query = numbers.SelectMany(num => letters.Select(letter => Tuple.Create(num, letter)));
        ...
        {
            var scheduler = new TestScheduler();
            var observer = scheduler.CreateObserver<Tuple<string, string>>();
            var s1 = new[]{
		        Tuple.Create(0,"zero"),
		        Tuple.Create(1,"one"),
		        Tuple.Create(2,"two"),
